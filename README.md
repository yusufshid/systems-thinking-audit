...
It works from whatever you have — a system prompt, `SKILL.md`/`CLAUDE.md`, a narrated description of the architecture, the actual codebase, an execution trace/log of real runs, or a previous audit report of the same system. You don't need all of them; see the skill's own Step 1 for what each is good for. Static material (prompt/code/config) tells you what a loop or a retry is *capable* of doing — a trace tells you what it actually did, which matters because feedback-loop and emergent-behavior findings are claims about interaction over time; the skill flags findings as trace-grounded vs. plausible-from-static-material rather than presenting both with the same confidence. And a previous report turns a one-off snapshot into a comparison — the report's Delta section separates what's actually resolved from what's still there, and a finding that keeps recurring across audits is treated as strong evidence of Shifting the Burden (see System Archetypes), not just a repeated observation.
